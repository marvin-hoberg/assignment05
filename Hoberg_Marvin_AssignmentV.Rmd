...
last_page <- venue_data1[["page"]][["totalPages"]]  # page number 629
...
for (p in first_page:(last_page-1)){
  ...
  df2 <- lapply(1:nrow(venue_data2_parsed), function(i) {
    ...
    data.frame(
      ...
      # check whether a name exists within nested list 
      city = df$city$name,
      postalCode = df$postalCode,
      address = ifelse("address" %in% names(df) == TRUE, df$address$line1, NA),  
      # check whether an address exists within nested list 
...
GER_duplicates <- janitor::get_dupes(df_venue_data2, "name", "address", "city", "postalCode")
...
easy_removables <- GER_duplicates[duplicated(GER_duplicates$lat, GER_duplicates$long), ]
...
GER_geo_opt <- tidy_data[between(tidy_data$long, GER_min_long, GER_max_long)&
...
ggplot() +
  ...
  geom_point(aes(x = long, y = lat),
    data = GER_geo_opt, color = "mediumpurple", size = 0.3) +
  theme_void() + 
  coord_quickmap() +
